   ...
   {ini_set('memory_limit', '500M');
    ...
    public function todo_load_list()
    {
        ...
        foreach ($list as $task) {
            ...
            $row[] = '<a class="btn-info btn-sm" href="edittask?id=' . $task->id . '" data-object-id="' . $task->id . '"> <i class="icon-pencil"></i> </a>&nbsp;<a class="btn-brown btn-sm delete-object" href="#" data-object-id="' . $task->id . '"> <i class="icon-trash-o"></i> </a>&nbsp; <a href="'.base_url().'manager/historial?id='.$task->id.'" data-id="' . $task->id . '" class="historial_task btn-sm btn-indigo" title="Historial"> <i class="icon-eye"></i> </a>';
    ...
    public function setgoals()
    {
        if ($this->input->post('income')) {
            ...
            if ($this->tools->setgoals($income, $expense, $sales, $netincome, $users,$vesagro,$servicios,$compras,$creditos,$nomina,$socios,$oficial)) {
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('UPDATED')));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
            }
