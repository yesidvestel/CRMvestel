...
<script type="text/javascript">
    ...
    listaProductos=[];
     $.post(baseurl + 'redes/stock_transfer_products?wid='+tips,{},function(data){
            
            $("#products_l").val("");
            $("#products_l").trigger("change");
            var options="";
            $(data).each(function(index,data2){
                options+="<option value='"+data2.id+"'>"+data2.serial+" - "+data2.marca+"</option>";
            });
            dataglobal=data;
    ...
    $("#products_l").on('select2:select',function(e){
                ...
                $("#remover_fila").html('');
                $("#itemsx").append('<tr id="fila_'+itemSeleccionado.id+'"> <td>'+itemSeleccionado.id+'</td><td>'+itemSeleccionado.serial+'</td> </tr>');
    ...
    function al_enviar_form(event){
		...
		console.log(listaProductos)
        $.post(baseurl+'redes/stock_transfer',{lista:listaProductos,from_warehouse:$("#wfrom").val(),to_warehouse:$("#sel2").val(),'observaciones_acta':observaciones},function(data){
