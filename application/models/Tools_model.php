   ...
   private function _task_datatables_query($cday = '')
    ...
    public function settask($id, $stat)
    {
        ...
        if($this->db->update('todolist')){
			...
			$content = 'Su Tarea a cambiado a estado '.$lan.' puedes revisar actividades en '.$url.'manager/historial?id='.$id;
    ...
    public function setgoals($income, $expense, $sales, $netincome, $users,$vesagro,$servicios,$compras,$creditos,$nomina,$socios,$oficial,$purchase,$internet,$programadora,$impuestos,$publicos,$comisiones,$celulares)
    {
...
public function obtener_notificaciones(){
        ...
        foreach ($lista_notificaciones as $key => $value) {
            ...
            $retorno['str_retorno'].= "<li>Tarea #".$value['id_tarea']." <i class='class_resuelta'><b>resuelta</b></i>, ".$name.$btn1;//&nbsp<a href="#" data-id="' . $value['id_tarea'] . '" class="view_task2 btn-sm btn-indigo"> <i class="icon-eye"></i> </a>
