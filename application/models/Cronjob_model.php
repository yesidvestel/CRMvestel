...
class Cronjob_model extends CI_Model
{
    ...
    public function realizar_proceso_de_reportes($year){
        ...
        $query = $this->db->query("SELECT MONTH(date) AS month,YEAR(date) AS year,SUM(credit) AS income,SUM(debit) AS expense FROM transactions WHERE (YEAR(date)='$year') and type!='Transfer' and account!='PAYU' and estado is null and tid!=-1 GROUP BY MONTH(date)");
